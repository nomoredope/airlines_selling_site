from flask import render_template, Blueprint, url_for, session, request, current_app
    ...
    if request.method == 'GET':
        ...
        if login:
            ...
            if user_info:
                ...
                return render_template('auth.html', user_group=user_dict['user_group'])
            else:
...
def define_user(login: str, password: str) -> Optional[Dict]:
    ...
    for sql_search in [sql_internal, sql_external]:
        ...
        if _user_info:
            ...
            del _user_info
            break
    return user_info #словарь ключом в котором является название столбца а значением его значение
